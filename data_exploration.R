...
# Import csv product descriptions regular 
product_description_regular <- read_delim("data/product_description_pre_owned.csv", 
                                            delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
# X brands occur in the dataset. This is too much to plot.
brand_n_reg <- product_description_regular %>%
  count(brand)
...
# X categories occur in the dataset. This is too much to plot.
category_n_reg <- product_description_regular %>%
  count(category)
...
# X color or color combinations occur in the dataset. This is too much to plot.
color_n_reg <- product_description_regular %>%
  count(color)
...
# X sizes occur in the dataset. This is too much too plot.
size_n_reg <- product_description_regular %>%
  count(size)

# Create a dataset with the sizes that occurs more than a 100 times. 
size_most_occur_reg <- size_n_reg %>%
  filter(n > 100)

ggplot(data = size_most_occur_reg, aes(x = size, y = n)) +
  geom_bar(stat = "identity", fill = "purple") +
  labs(title = "Sizes that occurs the most on Zalando Pre-owned",
       x = "Sizes", y = "Number of times in dataset")
